...
namespace System.Refactive.Subjects
{
    ...
    public static class Subject
    {
        ...
        public static ISubject<TSource, TResult> Create<TSource, TResult>(IRefObserver<TSource> observer, IRefObservable<TResult> observable)
        {
            if (observer == null)
            {
        ...
        /// <returns>Subject implemented using the given observer and observable.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="observer"/> or <paramref name="observable"/> is <c>null</c>.</exception>
        public static ISubject<T> Create<T>(IRefObserver<T> observer, IRefObservable<T> observable)
        {
            if (observer == null)
            {
                throw new ArgumentNullException(nameof(observer));
        ...
        private class AnonymousSubject<T, U> : ISubject<T, U>
        {
            ...
            private readonly IRefObservable<U> _observable;

            public AnonymousSubject(IRefObserver<T> observer, IRefObservable<U> observable)
            {
                _observer = observer;
            ...
            public void OnNext(ref T value) => _observer.OnNext(ref value);

            public IDisposable Subscribe(IRefObserver<U> observer)
            {
                if (observer == null)
                {
                    throw new ArgumentNullException(nameof(observer));
        ...
        {
            public AnonymousSubject(IRefObserver<T> observer, IRefObservable<T> observable)
