...
namespace System.Refactive.Subjects
{
    /// <summary>
    ...
    public sealed class ReplaySubject<T> : SubjectBase<T>
    {
        ...
        #region IRefObserver<T> implementation

        /// <summary>
        /// Notifies all subscribed and future observers about the arrival of the specified element in the sequence.
        /// </summary>
        /// <param name="value">The value to send to all observers.</param>
        public override void OnNext(ref T value) => _implementation.OnNext(ref value);
        ...
        #region IRefObservable<T> implementation
        ...
        private abstract class ReplayBase : SubjectBase<T>
        {
            ...
            protected abstract int Replay(IRefObserver<T> observer);

            protected abstract void Trim();

            protected abstract IScheduledObserver<T> CreateScheduledObserver(IRefObserver<T> observer);
        ...
        private sealed class ReplayByTime : ReplayBase
        {
            ...
            protected override IScheduledObserver<T> CreateScheduledObserver(IRefObserver<T> observer)
            {
            ...
            protected override int Replay(IRefObserver<T> observer)
            {
                var n = _queue.Count;

                foreach (var item in _queue)
                {
                    observer.OnNext(ref Unsafe.AsRef(item.Value));
                }
        ...
        private abstract class ReplayManyBase : ReplayBufferBase
        {
            ...
            protected override int Replay(IRefObserver<T> observer)
            {
                ...
                foreach (var item in _queue)
                {
                    observer.OnNext(ref Unsafe.AsRef(item));
                }
    ...
    internal sealed class FastImmediateObserver<T> : IScheduledObserver<T>
    {
        ...
        /// </summary>
        private volatile IRefObserver<T> _observer;
        ...
        public FastImmediateObserver(IRefObserver<T> observer)
        {
            _observer = observer;
        }
                ...
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        if (queue != null)
                        {
                            ...
                            while (queue.Count > 0)
                            {
                                _observer.OnNext(ref Unsafe.AsRef(queue.Dequeue()));
                            }
