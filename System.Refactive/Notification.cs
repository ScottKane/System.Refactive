...
namespace System.Refactive
{
    ...
    public abstract class Notification<T> : IEquatable<Notification<T>>
    {
        ...
        internal sealed class OnNextNotification : Notification<T>
        {
            ...
            /// </summary>
            public override string ToString() => string.Format(CultureInfo.CurrentCulture, "OnNext({0})", _value);

            /// <summary>
            /// Invokes the observer's method corresponding to the notification.
            /// </summary>
            /// <param name="observer">Observer to invoke the notification on.</param>
            public override void Accept(IRefObserver<T> observer)
            {
                if (observer == null)
                {
            ...
            public override TResult Accept<TResult>(IRefObserver<T, TResult> observer)
            {
                if (observer == null)
                {
                    throw new ArgumentNullException(nameof(observer));
                ...
                return observer.OnNext(ref _value);
            }
        ...
        /// </summary>
        /// <param name="observer">Observer to invoke the notification on.</param>
        public abstract void Accept(IRefObserver<T> observer);
        ...
        /// <param name="observer">Observer to invoke the notification on.</param>
        /// <returns>Result produced by the observation.</returns>
        public abstract TResult Accept<TResult>(IRefObserver<T, TResult> observer);
        ...
        public IRefObservable<T> ToObservable() => ToObservable(ImmediateScheduler.Instance);
        ...
        /// <returns>The observable sequence that surfaces the behavior of the notification upon subscription.</returns>
        public IRefObservable<T> ToObservable(IScheduler scheduler)
        {
            if (scheduler == null)
            {
