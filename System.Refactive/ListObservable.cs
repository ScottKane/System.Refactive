...
namespace System.Refactive
{
    ...
    [Experimental]
    public class ListObservable<T> : IList<T>, IRefObservable<object>
    {
        ...
        public ListObservable(IRefObservable<T> source)
        {
            if (source == null)
            {
            ...
            _subscription = source.Subscribe((ref T x) => _results.Add(x), _subject.OnError, _subject.OnCompleted);
        }
        ...
        public IDisposable Subscribe(IRefObserver<object> observer)
        {
            if (observer == null)
            {
