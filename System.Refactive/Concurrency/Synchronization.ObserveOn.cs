...
namespace System.Refactive.Concurrency
{
    internal static class ObserveOn<TSource>
    {
        ...
        internal sealed class Scheduler : Producer<TSource, ObserveOnObserverNew<TSource>>
        {
            ...
            public Scheduler(IRefObservable<TSource> source, IScheduler scheduler)
            {
                _source = source;
                _scheduler = scheduler;
            }

            protected override ObserveOnObserverNew<TSource> CreateSink(IRefObserver<TSource> observer) => new ObserveOnObserverNew<TSource>(_scheduler, observer);
        ...
        internal sealed class SchedulerLongRunning : Producer<TSource, ObserveOnObserverLongRunning<TSource>>
        {
            ...
            public SchedulerLongRunning(IRefObservable<TSource> source, ISchedulerLongRunning scheduler)
            {
                _source = source;
                _scheduler = scheduler;
            }

            protected override ObserveOnObserverLongRunning<TSource> CreateSink(IRefObserver<TSource> observer) => new ObserveOnObserverLongRunning<TSource>(_scheduler, observer);
        ...
        internal sealed class Context : Producer<TSource, Context._>
        {
            ...
            public Context(IRefObservable<TSource> source, SynchronizationContext context)
            {
                _source = source;
                _context = context;
            }

            protected override _ CreateSink(IRefObserver<TSource> observer) => new _(_context, observer);
            ...
            internal sealed class _ : IdentitySink<TSource>
            {
                ...
                public _(SynchronizationContext context, IRefObserver<TSource> observer)
                    : base(observer)
                {
                    _context = context;
                }
                ...
                {
                    ForwardOnNext(ref Unsafe.AsRef<TSource>((TSource)value!));
                }
