...
namespace System.Refactive.Internal
{
    ...
    internal interface IProducer<TSource> : IRefObservable<TSource>
    {
        IDisposable SubscribeRaw(IRefObserver<TSource> observer, bool enableSafeguard);
    }
    ...
    internal abstract class BasicProducer<TSource> : IProducer<TSource>
    {
        ...
        public IDisposable Subscribe(IRefObserver<TSource> observer)
        {
            if (observer == null)
            {
        ...
        public IDisposable SubscribeRaw(IRefObserver<TSource> observer, bool enableSafeguard)
        {
            IDisposable run;
        ...
        protected abstract IDisposable Run(IRefObserver<TSource> observer);
    }
        ...
        where TSink : IDisposable
    {
        ...
        /// <param name="observer">Observer to send notifications on. The implementation of a producer must ensure the correct message grammar on the observer.</param>
        /// <returns>IDisposable to cancel the subscription. This causes the underlying sink to be notified of unsubscription, causing it to prevent further messages from being sent to the observer.</returns>
        public IDisposable Subscribe(IRefObserver<TTarget> observer)
        {
            if (observer == null)
            {
                throw new ArgumentNullException(nameof(observer));
        ...
        public IDisposable SubscribeRaw(IRefObserver<TTarget> observer, bool enableSafeguard)
        {
        ...
        protected abstract TSink CreateSink(IRefObserver<TTarget> observer);
    }
}
