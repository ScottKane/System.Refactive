...
namespace System.Refactive.Internal
{
    ...
    {
        private readonly Stack<IEnumerator<IRefObservable<TSource>>> _stack = new Stack<IEnumerator<IRefObservable<TSource>>>();

        private bool _isDisposed;
        private int _trampoline;
        private IDisposable? _currentSubscription;

        protected TailRecursiveSink(IRefObserver<TSource> observer)
            : base(observer)
        {
        }
        ...
        private void Drain()
        {
            ...
            for (; ; )
            {
                ...
                else
                {
                    if (_stack.Count != 0)
                    {
                        ...
                        var currentObservable = default(IRefObservable<TSource>);
            ...
            static IRefObservable<T>? Unpack<T>(IRefObservable<T>? source)
            {
                bool hasOpt;
        ...
        protected abstract IEnumerable<IRefObservable<TSource>>? Extract(IRefObservable<TSource> source);

        private bool TryGetEnumerator(IEnumerable<IRefObservable<TSource>> sources, [NotNullWhen(true)] out IEnumerator<IRefObservable<TSource>>? result)
        {
            try
            {
