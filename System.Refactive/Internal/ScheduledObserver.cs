...
namespace System.Refactive.Internal
{
    internal class ScheduledObserver<T> : ObserverBase<T>, IScheduledObserver<T>
    {
        ...
        public ScheduledObserver(IScheduler scheduler, IRefObserver<T> observer)
        {
        ...
        private void EnsureActiveSlow()
        {
            ...
            while (true)
            {
                ...
                // should only be called after invocation of IRefObserver<T> methods that touch
                // this state.
                //
    ...
    internal sealed class ObserveOnObserver<T> : ScheduledObserver<T>
    {
        ...
        public ObserveOnObserver(IScheduler scheduler, IRefObserver<T> observer)
            : base(scheduler, observer)
        {

        }

        public void Run(IRefObservable<T> source)
        {
    ...
    internal interface IScheduledObserver<T> : IRefObserver<T>, IDisposable
    {
        void EnsureActive();
    ...
    internal sealed class ObserveOnObserverNew<T> : IdentitySink<T>
    {
        ...
        public ObserveOnObserverNew(IScheduler scheduler, IRefObserver<T> downstream) : base(downstream)
        {
            _scheduler = scheduler;
    ...
    internal sealed class ObserveOnObserverLongRunning<TSource> : IdentitySink<TSource>
    {
        ...
        public ObserveOnObserverLongRunning(ISchedulerLongRunning scheduler, IRefObserver<TSource> observer) : base(observer)
        {
            _scheduler = scheduler;
