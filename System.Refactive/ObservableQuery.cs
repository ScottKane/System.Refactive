...
namespace System.Refactive
{
    internal class ObservableQueryProvider : IQbservableProvider, IQueryProvider
    {
        public IQbservable<TResult> CreateQuery<TResult>(Expression expression)
        {
            ...
            if (!typeof(IRefObservable<TResult>).IsAssignableFrom(expression.Type))
            {
    ...
    {
        internal ObservableQuery(IRefObservable<TSource> source)
            : base(source)
        {
        }
        ...
        public IDisposable Subscribe(IRefObserver<TSource> observer)
        {
            if (_source == null)
            {
                ...
                var f = Expression.Lambda<Func<IRefObservable<TSource>>>(body);
                _source = f.Compile()();
            }

            //
            // [OK] Use of unsafe Subscribe: non-pretentious mapping to IRefObservable<T> behavior equivalent to the expression tree.
            //
            return ((IRefObservable<TSource>)_source).Subscribe/*Unsafe*/(observer);
        }
