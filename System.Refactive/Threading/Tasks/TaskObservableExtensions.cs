...
using System.Refactive.Linq;
using System.Runtime.CompilerServices;
using System.Threading;
using System.Threading.Tasks;

namespace System.Refactive.Threading.Tasks
{
    /// <summary>
    ...
    {
        private sealed class SlowTaskObservable : IRefObservable<Unit>
        {
            ...
            public IDisposable Subscribe(IRefObserver<Unit> observer)
            {
                if (observer == null)
                {
                ...
                if (_scheduler == null)
                {
                    _task.ContinueWith(static (t, subjectObject) => t.EmitTaskResult((IRefObserver<Unit>)subjectObject!), observer, cts.Token, options, TaskScheduler.Current);
                }
                else
                {
        ...
        private sealed class SlowTaskObservable<TResult> : IRefObservable<TResult>
        {
            ...
            public IDisposable Subscribe(IRefObserver<TResult> observer)
            {
                ...
                if (_scheduler == null)
                {
                    _task.ContinueWith(static (t, subjectObject) => t.EmitTaskResult((IRefObserver<TResult>)subjectObject!), observer, cts.Token, options, TaskScheduler.Current);
                }
                else
                {
                    _task.ContinueWithState(
        ...
        /// <remarks>If the specified task object supports cancellation, consider using <see cref="Observable.FromAsync(System.Func{System.Threading.CancellationToken,System.Threading.Tasks.Task})"/> instead.</remarks>
        public static IRefObservable<Unit> ToObservable(this Task task)
        {
            if (task == null)
            {
        ...
        public static IRefObservable<Unit> ToObservable(this Task task, IScheduler scheduler)
        {
            if (task == null)
            {
                throw new ArgumentNullException(nameof(task));
        ...
        private static IRefObservable<Unit> ToObservableImpl(Task task, IScheduler? scheduler)
        {
            if (task.IsCompleted)
            {
        ...
        private static void EmitTaskResult(this Task task, IRefObserver<Unit> subject)
        {
            switch (task.Status)
            {
                case TaskStatus.RanToCompletion:
                    subject.OnNext(ref Unsafe.AsRef(Unit.Default));
                    subject.OnCompleted();
                    break;
        ...
        internal static IDisposable Subscribe(this Task task, IRefObserver<Unit> observer)
        {
            if (task.IsCompleted)
            {
            ...
            task.ContinueWith(
                static (t, observerObject) => t.EmitTaskResult((IRefObserver<Unit>)observerObject!), 
                observer, 
                cts.Token, 
        ...
        public static IRefObservable<TResult> ToObservable<TResult>(this Task<TResult> task)
        {
            if (task == null)
            {
                throw new ArgumentNullException(nameof(task));
        ...
        /// <exception cref="ArgumentNullException"><paramref name="task"/> is <c>null</c> or <paramref name="scheduler"/> is <c>null</c>.</exception>
        /// <remarks>If the specified task object supports cancellation, consider using <see cref="Observable.FromAsync{TResult}(Func{CancellationToken, Task{TResult}})"/> instead.</remarks>
        public static IRefObservable<TResult> ToObservable<TResult>(this Task<TResult> task, IScheduler scheduler)
        {
            if (task == null)
            {
                throw new ArgumentNullException(nameof(task));
        ...
        private static IRefObservable<TResult> ToObservableImpl<TResult>(Task<TResult> task, IScheduler? scheduler)
        {
            if (task.IsCompleted)
            {
                scheduler ??= ImmediateScheduler.Instance;
        ...
        private static void EmitTaskResult<TResult>(this Task<TResult> task, IRefObserver<TResult> subject)
        {
            switch (task.Status)
            {
                case TaskStatus.RanToCompletion:
                    subject.OnNext(ref Unsafe.AsRef(task.Result));
                    subject.OnCompleted();
                    break;
                case TaskStatus.Faulted:
                    subject.OnError(task.GetSingleException());
        ...
        internal static IDisposable Subscribe<TResult>(this Task<TResult> task, IRefObserver<TResult> observer)
        {
            if (task.IsCompleted)
            {
                task.EmitTaskResult(observer);
            ...
            task.ContinueWith(
                static (t, observerObject) => t.EmitTaskResult((IRefObserver<TResult>)observerObject!), 
                observer, 
                cts.Token, 
                TaskContinuationOptions.ExecuteSynchronously, 
                TaskScheduler.Current);
        ...
        public static Task<TResult> ToTask<TResult>(this IRefObservable<TResult> observable)
        {
            if (observable == null)
            {
        ...
        public static Task<TResult> ToTask<TResult>(this IRefObservable<TResult> observable, IScheduler scheduler)
        {
        ...
        /// <returns>A task that will receive the last element or the exception produced by the observable sequence.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="observable"/> is <c>null</c>.</exception>
        public static Task<TResult> ToTask<TResult>(this IRefObservable<TResult> observable, object? state)
        {
            if (observable == null)
            {
                throw new ArgumentNullException(nameof(observable));
        ...
        /// <returns>A task that will receive the last element or the exception produced by the observable sequence.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="observable"/> or <paramref name="scheduler"/> is <c>null</c>.</exception>
        public static Task<TResult> ToTask<TResult>(this IRefObservable<TResult> observable, object? state, IScheduler scheduler)
        {
        ...
        /// <returns>A task that will receive the last element or the exception produced by the observable sequence.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="observable"/> is <c>null</c>.</exception>
        public static Task<TResult> ToTask<TResult>(this IRefObservable<TResult> observable, CancellationToken cancellationToken)
        {
            if (observable == null)
            {
                throw new ArgumentNullException(nameof(observable));
        ...
        /// <returns>A task that will receive the last element or the exception produced by the observable sequence.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="observable"/> or <paramref name="scheduler"/> is <c>null</c>.</exception>
        public static Task<TResult> ToTask<TResult>(this IRefObservable<TResult> observable, CancellationToken cancellationToken, IScheduler scheduler)
        {
        ...
        /// <returns>A task that will receive the last element or the exception produced by the observable sequence.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="observable"/> is <c>null</c>.</exception>
        public static Task<TResult> ToTask<TResult>(this IRefObservable<TResult> observable, CancellationToken cancellationToken, object? state)
        {
            if (observable == null)
            {
                throw new ArgumentNullException(nameof(observable));
        ...
        /// <returns>A task that will receive the last element or the exception produced by the observable sequence.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="observable"/> or <paramref name="scheduler"/> is <c>null</c>.</exception>
        public static Task<TResult> ToTask<TResult>(this IRefObservable<TResult> observable, CancellationToken cancellationToken, object? state, IScheduler scheduler)
        {
