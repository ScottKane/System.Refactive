...
namespace System.Refactive.Linq
{
    internal sealed class GroupByUntil<TSource, TKey, TElement, TDuration> : Producer<IGroupedObservable<TKey, TElement>, GroupByUntil<TSource, TKey, TElement, TDuration>._>
    {
        ...
        private readonly Func<IGroupedObservable<TKey, TElement>, IRefObservable<TDuration>> _durationSelector;
        private readonly int? _capacity;
        private readonly IEqualityComparer<TKey> _comparer;

        public GroupByUntil(IRefObservable<TSource> source, Func<TSource, TKey> keySelector, Func<TSource, TElement> elementSelector, Func<IGroupedObservable<TKey, TElement>, IRefObservable<TDuration>> durationSelector, int? capacity, IEqualityComparer<TKey> comparer)
        {
            _source = source;
        ...
        protected override _ CreateSink(IRefObserver<IGroupedObservable<TKey, TElement>> observer) => new _(this, observer);
        ...
        internal sealed class _ : Sink<TSource, IGroupedObservable<TKey, TElement>>
        {
            ...
            public _(GroupByUntil<TSource, TKey, TElement, TDuration> parent, IRefObserver<IGroupedObservable<TKey, TElement>> observer)
                : base(observer)
            {
            ...
            public override void OnNext(ref TSource value)
            {
                ...
                if (fireNewMapEntry)
                {
                    ...
                    IRefObservable<TDuration> duration;
                    try
                    {
                    ...
                    lock (_gate)
                    {
                        ForwardOnNext(ref Unsafe.AsRef<IGroupedObservable<TKey, TElement>>(group));
                    }
            ...
            private sealed class DurationObserver : SafeObserver<TDuration>
            {
                ...
                public override void OnNext(ref TDuration value)
                {
                    OnCompleted();
                }
