...
namespace System.Refactive.Linq
{
    internal sealed class FromEventPattern
    {
        public sealed class Impl<TDelegate, TEventArgs> : ClassicEventProducer<TDelegate, EventPattern<TEventArgs>>
        {
            ...
            protected override TDelegate GetHandler(RefAction<EventPattern<TEventArgs>> onNext)
            {
                TDelegate handler;

                if (_conversion == null)
                {
                    Action<object, TEventArgs> h = (sender, eventArgs) => onNext(ref Unsafe.AsRef(new EventPattern<TEventArgs>(sender, eventArgs)));
                ...
                {
                    handler = _conversion((sender, eventArgs) => onNext(ref Unsafe.AsRef(new EventPattern<TEventArgs>(sender, eventArgs))));
                }
        ...
        public sealed class Impl<TDelegate, TSender, TEventArgs> : ClassicEventProducer<TDelegate, EventPattern<TSender, TEventArgs>>
        {
            ...
            protected override TDelegate GetHandler(RefAction<EventPattern<TSender, TEventArgs>> onNext)
            {
                Action<TSender, TEventArgs> h = (sender, eventArgs) => onNext(ref Unsafe.AsRef(new EventPattern<TSender, TEventArgs>(sender, eventArgs)));
        ...
        public sealed class Handler<TSender, TEventArgs, TResult> : EventProducer<Delegate, TResult>
        {
            ...
            protected override Delegate GetHandler(RefAction<TResult> onNext)
            {
                Action<TSender, TEventArgs> h = (sender, eventArgs) => onNext(ref Unsafe.AsRef(_getResult(sender, eventArgs)));
