...
namespace System.Refactive.Linq
{
    internal static class Delay<TSource>
    {
            ...
            where TParent : Base<TParent>
        {
            ...
            protected Base(IRefObservable<TSource> source, IScheduler scheduler)
            {
                _source = source;
            ...
            internal abstract class _ : IdentitySink<TSource>
            {
                ...
                protected _(TParent parent, IRefObserver<TSource> observer)
                    : base(observer)
                {
            ...
            internal abstract class S : _
            {
                ...
                protected S(TParent parent, IRefObserver<TSource> observer)
                ...
                protected Queue<Refactive.TimeInterval<TSource>> _queue = new Queue<Refactive.TimeInterval<TSource>>();
                ...
                public override void OnNext(ref TSource value)
                {
                    ...
                    lock (_gate)
                    {
                        ...
                        _queue.Enqueue(new Refactive.TimeInterval<TSource>(value, next));
            ...
            protected abstract class L : _
            {
                ...
                protected L(TParent parent, IRefObserver<TSource> observer)
                    : base(parent, observer)
                {
                }
        ...
        internal sealed class Absolute : Base<Absolute>
        {
            ...
            protected override _ CreateSink(IRefObserver<TSource> observer) => _scheduler.AsLongRunning() != null ? (_)new L(this, observer) : new S(this, observer);

            protected override void Run(_ sink) => sink.Run(this);

            private new sealed class S : Base<Absolute>.S
            {
                public S(Absolute parent, IRefObserver<TSource> observer)
                    : base(parent, observer)
                {
                }
                    ...
                    lock (_gate)
                    {
                        ...
                        if (oldQueue.Count > 0)
                        {
                            ...
                            while (oldQueue.Count > 0)
                            {
                                ...
                                _queue.Enqueue(new Refactive.TimeInterval<TSource>(item.Value, item.Interval.Add(_delay)));
                            }
            ...
            {
                public L(Absolute parent, IRefObserver<TSource> observer)
                    : base(parent, observer)
                {
                }
        ...
        internal sealed class Relative : Base<Relative>
        {
            ...
            {
                public S(Relative parent, IRefObserver<TSource> observer)
                    : base(parent, observer)
                {
                }
            ...
            {
                public L(Relative parent, IRefObserver<TSource> observer)
                    : base(parent, observer)
                {
                }
    ...
    internal static class Delay<TSource, TDelay>
    {
            ...
            where TParent : Base<TParent>
        {
            ...
            protected Base(IRefObservable<TSource> source)
            {
                _source = source;
            }
            ...
            internal abstract class _ : IdentitySink<TSource>
            {
                ...
                private readonly Func<TSource, IRefObservable<TDelay>> _delaySelector;

                protected _(Func<TSource, IRefObservable<TDelay>> delaySelector, IRefObserver<TSource> observer)
                    : base(observer)
                {
                ...
                private sealed class DelayObserver : SafeObserver<TDelay>
                {
                    ...
                    public override void OnNext(ref TDelay value)
                    {
                        if (!_once)
                        {
                            _once = true;
                            lock (_parent._gate)
                            {
                                _parent.ForwardOnNext(ref _value);
        ...
        internal class Selector : Base<Selector>
        {
            ...
            public Selector(IRefObservable<TSource> source, Func<TSource, IRefObservable<TDelay>> delaySelector)
                : base(source)
            {
                _delaySelector = delaySelector;
            }

            protected override Base<Selector>._ CreateSink(IRefObserver<TSource> observer) => new _(_delaySelector, observer);

            protected override void Run(Base<Selector>._ sink) => sink.Run(this);

            private new sealed class _ : Base<Selector>._
            {
                public _(Func<TSource, IRefObservable<TDelay>> delaySelector, IRefObserver<TSource> observer)
        ...
        {
            private readonly IRefObservable<TDelay> _subscriptionDelay;
            private readonly Func<TSource, IRefObservable<TDelay>> _delaySelector;

            public SelectorWithSubscriptionDelay(IRefObservable<TSource> source, IRefObservable<TDelay> subscriptionDelay, Func<TSource, IRefObservable<TDelay>> delaySelector)
                : base(source)
            {
            ...
            protected override Base<SelectorWithSubscriptionDelay>._ CreateSink(IRefObserver<TSource> observer) => new _(_delaySelector, observer);
            ...
            private new sealed class _ : Base<SelectorWithSubscriptionDelay>._
            {
                ...
                private sealed class SubscriptionDelayObserver : IRefObserver<TDelay>, IDisposable
                {
                    private readonly _ _parent;
                    private readonly IRefObservable<TSource> _source;
                    private SerialDisposableValue _subscription;

                    public SubscriptionDelayObserver(_ parent, IRefObservable<TSource> source)
                    {
                        _parent = parent;
                        _source = source;
                    }
                    ...
                    public void OnNext(ref TDelay value)
                    {
