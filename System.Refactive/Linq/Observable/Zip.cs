...
namespace System.Refactive.Linq
{
    ...
    internal static class Zip<TFirst, TSecond, TResult>
    {
        internal sealed class Observable : Producer<TResult, Observable._>
        {
            ...
            public Observable(IRefObservable<TFirst> first, IRefObservable<TSecond> second, Func<TFirst, TSecond, TResult> resultSelector)
            {
                _first = first;
                _second = second;
            ...
            internal sealed class _ : IdentitySink<TResult>
            {
                ...
                private sealed class FirstObserver : IRefObserver<TFirst>, IDisposable
                {
                ...
                private sealed class SecondObserver : IRefObserver<TSecond>, IDisposable
                {
                    private readonly _ _parent;
        ...
        internal sealed class Enumerable : Producer<TResult, Enumerable._>
        {
            ...
            public Enumerable(IRefObservable<TFirst> first, IEnumerable<TSecond> second, Func<TFirst, TSecond, TResult> resultSelector)
            {
                _first = first;
                _second = second;
                _resultSelector = resultSelector;
            ...
            internal sealed class _ : Sink<TFirst, TResult>
            {
                ...
                public void Run(IRefObservable<TFirst> first, IEnumerable<TSecond> second)
                {
                    //
                ...
                public override void OnNext(ref TFirst value)
                {
    ...
    internal abstract class ZipSink<TResult> : IdentitySink<TResult>, IZip
    {
        ...
        protected ZipSink(int arity, IRefObserver<TResult> observer)
            : base(observer)
        {
            _gate = new object();
    ...
    internal sealed class Zip<TSource> : Producer<IList<TSource>, Zip<TSource>._>
    {
        ...
        public Zip(IEnumerable<IRefObservable<TSource>> sources)
        {
            _sources = sources;
        }
        ...
        internal sealed class _ : IdentitySink<IList<TSource>>
        {
            ...
            public void Run(IEnumerable<IRefObservable<TSource>> sources)
            {
            ...
            private sealed class SourceObserver : IRefObserver<TSource>
            {
                private readonly _ _parent;
                private readonly int _index;
