...
{
    internal sealed class ConcatMany<T> : IRefObservable<T>
    {
        private readonly IRefObservable<IRefObservable<T>> _sources;

        internal ConcatMany(IRefObservable<IRefObservable<T>> sources)
        {
            _sources = sources;
        }
        ...
        internal sealed class ConcatManyOuterObserver : IRefObserver<IRefObservable<T>>, IDisposable
        {
            private readonly IRefObserver<T> _downstream;
            private readonly ConcurrentQueue<IRefObservable<T>> _queue;
            ...
            internal ConcatManyOuterObserver(IRefObserver<T> downstream)
            {
                _downstream = downstream;
                _queue = new ConcurrentQueue<IRefObservable<T>>();
            ...
            internal sealed class InnerObserver : IRefObserver<T>, IDisposable
            {
                ...
                public void OnNext(ref T value) => _parent.InnerNext(value);
            }
        }
    }
}
