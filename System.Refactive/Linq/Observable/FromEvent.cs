...
namespace System.Refactive.Linq
{
    ...
    {
        private readonly Func<RefAction<TEventArgs>, TDelegate>? _conversion;
        ...
        public FromEvent(Func<RefAction<TEventArgs>, TDelegate> conversion, Action<TDelegate> addHandler, Action<TDelegate> removeHandler, IScheduler scheduler)
            : base(addHandler, removeHandler, scheduler)
        {
            _conversion = conversion;
        }

        protected override TDelegate GetHandler(RefAction<TEventArgs> onNext)
        {
            TDelegate handler;
    ...
    internal abstract class EventProducer<TDelegate, TArgs> : BasicProducer<TArgs>
    {
        ...
        protected abstract TDelegate GetHandler(RefAction<TArgs> onNext);
        protected abstract IDisposable AddHandler(TDelegate handler);

        private Session? _session;

        protected override IDisposable Run(IRefObserver<TArgs> observer)
        {
        ...
        private sealed class Session
        {
            ...
            public IDisposable Connect(IRefObserver<TArgs> observer)
            {
                /*
