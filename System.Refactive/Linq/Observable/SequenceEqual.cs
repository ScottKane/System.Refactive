...
namespace System.Refactive.Linq
{
    internal static class SequenceEqual<TSource>
    {
        ...
        {
            private readonly IRefObservable<TSource> _first;
            private readonly IRefObservable<TSource> _second;
            private readonly IEqualityComparer<TSource> _comparer;

            public Observable(IRefObservable<TSource> first, IRefObservable<TSource> second, IEqualityComparer<TSource> comparer)
            {
                _first = first;
                _second = second;
                _comparer = comparer;
            ...
            protected override _ CreateSink(IRefObserver<bool> observer) => new _(_comparer, observer);
            ...
            internal sealed class _ : IdentitySink<bool>
            {
                ...
                public _(IEqualityComparer<TSource> comparer, IRefObserver<bool> observer)
                    : base(observer)
                {
                    _comparer = comparer;
                    _gate = new object();
                ...
                private sealed class FirstObserver : IRefObserver<TSource>
                {
                    ...
                    public void OnNext(ref TSource value)
                    {
                        lock (_parent._gate)
                        {
                            if (_parent._qr.Count > 0)
                            {
                                ...
                                if (!equal)
                                {
                                    _parent.ForwardOnNext(ref Unsafe.AsRef(false));
                    ...
                    public void OnCompleted()
                    {
                        lock (_parent._gate)
                        {
                            ...
                            if (_parent._ql.Count == 0)
                            {
                                ...
                                {
                                    _parent.ForwardOnNext(ref Unsafe.AsRef(true));
                                    _parent.ForwardOnCompleted();
                                }
                            }
                        }
                ...
                private sealed class SecondObserver : IRefObserver<TSource>
                {
                    private readonly _ _parent;
        ...
        internal sealed class Enumerable : Producer<bool, Enumerable._>
        {
            ...
            public Enumerable(IRefObservable<TSource> first, IEnumerable<TSource> second, IEqualityComparer<TSource> comparer)
            {
                _first = first;
                _second = second;
                _comparer = comparer;
            ...
            internal sealed class _ : Sink<TSource, bool>
            {
                ...
                public override void OnCompleted()
                {
                    ...
                    ForwardOnNext(ref Unsafe.AsRef(!hasNext));
                    ForwardOnCompleted();
                }
            }
        }
