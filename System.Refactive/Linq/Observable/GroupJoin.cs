...
namespace System.Refactive.Linq
{
    ...
    {
        private readonly IRefObservable<TLeft> _left;
        private readonly IRefObservable<TRight> _right;
        private readonly Func<TLeft, IRefObservable<TLeftDuration>> _leftDurationSelector;
        private readonly Func<TRight, IRefObservable<TRightDuration>> _rightDurationSelector;
        private readonly Func<TLeft, IRefObservable<TRight>, TResult> _resultSelector;

        public GroupJoin(IRefObservable<TLeft> left, IRefObservable<TRight> right, Func<TLeft, IRefObservable<TLeftDuration>> leftDurationSelector, Func<TRight, IRefObservable<TRightDuration>> rightDurationSelector, Func<TLeft, IRefObservable<TRight>, TResult> resultSelector)
        {
            _left = left;
            _right = right;
        ...
        internal sealed class _ : IdentitySink<TResult>
        {
            ...
            private readonly SortedDictionary<int, IRefObserver<TRight>> _leftMap;
            ...
            public _(GroupJoin<TLeft, TRight, TLeftDuration, TRightDuration, TResult> parent, IRefObserver<TResult> observer)
                : base(observer)
            {
                _refCount = new RefCountDisposable(_group);
                _leftMap = new SortedDictionary<int, IRefObserver<TRight>>();
            ...
            private sealed class LeftObserver : SafeObserver<TLeft>
            {
                ...
                private void Expire(int id, IRefObserver<TRight> group, IDisposable resource)
                {
                    lock (_parent._gate)
                    {
                ...
                public override void OnNext(ref TLeft value)
                {
                    ...
                    IRefObservable<TLeftDuration> duration;
                    try
                    {
                    ...
                    lock (_parent._gate)
                    {
                        _parent.ForwardOnNext(ref result);

                        foreach (var rightValue in _parent._rightMap)
                        {
                            if (rightValue.Key < rightID)
                            {
                                s.OnNext(ref Unsafe.AsRef(rightValue.Value));
                            }
                        }
                    }
                }
                ...
                private sealed class DurationObserver : SafeObserver<TLeftDuration>
                {
                    ...
                    private readonly IRefObserver<TRight> _group;

                    public DurationObserver(LeftObserver parent, int id, IRefObserver<TRight> group)
                    {
                        _parent = parent;
                        _id = id;
                        _group = group;
                    ...
                    public override void OnNext(ref TLeftDuration value)
                    {
            ...
            private sealed class RightObserver : SafeObserver<TRight>
            {
                ...
                public override void OnNext(ref TRight value)
                {
                    int id;
                    int leftID;
                    ...
                    IRefObservable<TRightDuration> duration;
                    try
                    {
                ...
                private sealed class DurationObserver : SafeObserver<TRightDuration>
                {
                    ...
                    public override void OnNext(ref TRightDuration value)
                    {
