...
namespace System.Refactive.Linq
{
    internal sealed class GroupBy<TSource, TKey, TElement> : Producer<IGroupedObservable<TKey, TElement>, GroupBy<TSource, TKey, TElement>._>
    {
        ...
        public GroupBy(IRefObservable<TSource> source, Func<TSource, TKey> keySelector, Func<TSource, TElement> elementSelector, int? capacity, IEqualityComparer<TKey> comparer)
        {
            _source = source;
        ...
        internal sealed class _ : Sink<TSource, IGroupedObservable<TKey, TElement>>
        {
            ...
            public _(GroupBy<TSource, TKey, TElement> parent, IRefObserver<IGroupedObservable<TKey, TElement>> observer)
                : base(observer)
            {
            ...
            public override void OnNext(ref TSource value)
            {
                ...
                if (fireNewMapEntry)
                {
                    ForwardOnNext(ref Unsafe.AsRef<IGroupedObservable<TKey, TElement>>(new GroupedObservable<TKey, TElement>(key, writer, _refCountDisposable!)));
                }
