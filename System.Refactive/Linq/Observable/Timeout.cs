...
namespace System.Refactive.Linq
{
    internal static class Timeout<TSource>
    {
        internal sealed class Relative : Producer<TSource, Relative._>
        {
            ...
            private readonly IRefObservable<TSource> _other;
            private readonly IScheduler _scheduler;

            public Relative(IRefObservable<TSource> source, TimeSpan dueTime, IRefObservable<TSource> other, IScheduler scheduler)
            {
                _source = source;
                _dueTime = dueTime;
                _other = other;
            ...
            internal sealed class _ : IdentitySink<TSource>
            {
                ...
                public _(Relative parent, IRefObserver<TSource> observer)
                    : base(observer)
                {
        ...
        internal sealed class Absolute : Producer<TSource, Absolute._>
        {
            ...
            public Absolute(IRefObservable<TSource> source, DateTimeOffset dueTime, IRefObservable<TSource> other, IScheduler scheduler)
            {
                _source = source;
                _dueTime = dueTime;
                _other = other;
            ...
            protected override _ CreateSink(IRefObserver<TSource> observer) => new _(_other, observer);
            ...
            internal sealed class _ : IdentitySink<TSource>
            {
                ...
                public _(IRefObservable<TSource> other, IRefObserver<TSource> observer)
                    : base(observer)
                {
                    _other = other;
                }
    ...
    {
        private readonly IRefObservable<TSource> _source;
        private readonly IRefObservable<TTimeout> _firstTimeout;
        private readonly Func<TSource, IRefObservable<TTimeout>> _timeoutSelector;
        private readonly IRefObservable<TSource> _other;

        public Timeout(IRefObservable<TSource> source, IRefObservable<TTimeout> firstTimeout, Func<TSource, IRefObservable<TTimeout>> timeoutSelector, IRefObservable<TSource> other)
        {
            _source = source;
        ...
        internal sealed class _ : IdentitySink<TSource>
        {
            ...
            public _(Timeout<TSource, TTimeout> parent, IRefObserver<TSource> observer)
                : base(observer)
            {
            ...
            public override void OnNext(ref TSource value)
            {
                ...
                if (idx != long.MaxValue)
                {
                    if (Interlocked.CompareExchange(ref _index, idx + 1, idx) == idx)
                    {
                        ...
                        IRefObservable<TTimeout> timeoutSource;
                        try
                        {
            ...
            private void SetTimer(IRefObservable<TTimeout> timeout, long idx)
            {
            ...
            private sealed class TimeoutObserver : SafeObserver<TTimeout>
            {
                ...
                public override void OnNext(ref TTimeout value)
                {
                    OnCompleted();
                }
