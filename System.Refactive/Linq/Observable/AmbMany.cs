...
namespace System.Refactive.Linq
{
    ...
    {
        private readonly IRefObservable<T>[] _sources;

        public AmbManyArray(IRefObservable<T>[] sources)
        {
            _sources = sources;
        }

        protected override IDisposable Run(IRefObserver<T> observer)
        {
    ...
    {
        private readonly IEnumerable<IRefObservable<T>> _sources;

        public AmbManyEnumerable(IEnumerable<IRefObservable<T>> sources)
        {
            _sources = sources;
        }
    ...
    internal sealed class AmbCoordinator<T> : IDisposable
    {
        ...
        internal AmbCoordinator(IRefObserver<T> downstream, int n)
        {
        ...
        internal static IDisposable Create(IRefObserver<T> observer, IRefObservable<T>[] sources)
        {
        ...
        internal void Subscribe(IRefObservable<T>[] sources)
        {
